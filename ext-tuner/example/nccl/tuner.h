...
static float nccl_nvlink_lat[NCCL_NUM_ALGORITHMS][NCCL_NUM_PROTOCOLS] = {
       { .6, 1.25, 2.8 }, /* Tree (LL/LL128/Simple)*/
...
static float nccl_net_lat[NCCL_NUM_ALGORITHMS][NCCL_NUM_PROTOCOLS] = {
       { 6.5,  10, 15 }, /* Tree (LL/LL128/Simple)*/
       { 2.7, 4.0, 14 }, /* Ring (LL/LL128/Simple)*/
       {   0,   0, 31 }, /* CollNetDirect (Simple)*/
       {   0,   0, 30 }, /* CollNetChain (Simple)*/
       {   0,   0, 18 }, /* NVLS (Simple) */
       {   0,   0, 15 }  /* NVLSTree (Simple) */
};

#define NCCL_TUNER_NET_LAT		(3)
#define NCCL_TUNER_NET_NUM_CHANNELS	(16)
#define NCCL_TUNER_INTERNODE_BW		(50.0)
 ...
 */
#define NCCL_TUNER_INTRANODE_BW		(20.0)
                                  ...
                                  ncclFunc_t func, int algo, int proto, int pipe_ops, size_t size)
{
	...
	interLat = nccl_net_lat[algo][proto];
	...
	cost = (latency * pipe_ops) + size / (1000 * bw);
	//printf("algo %d, proto %d, cost %.2f, lat %.2f, bw %.2f\n", algo, proto, cost, latency, bw);
